...
signed main() {
    ...
    int ans = 0;
    rep(i, adj.size()) {
        int st = adj[i].fst.fst;
        int en = adj[i].fst.snd;
        int val = adj[i].snd;
        int cur = val;
        if(lb(all(dq), pii(en, 0), compare2) == dq.end()) {
            dq.push_front(mp(st, max(mx, cur)));
            mx = max(mx, cur);
            ans = max(ans, max(mx,cur));
        }
        else {
            int idx = lb(all(dq), pii(en, 0), compare2) - dq.begin();
            cur += dq[idx].snd;
            mx = max(mx, cur);
            dq.push_front(mp(st, max(mx, cur)));
            ans = max(ans, max(mx,cur));
