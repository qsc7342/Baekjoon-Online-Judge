...
signed main() {
    ...
    int pt3 = 0;
    int pt5 = 0;
    eat3 = min((int)w/3, (int)three.size());
    for(int i = 0 ; i < eat3 ; i++) {
        cur += three[pt3++];
        ans = max(ans, cur);
    }

    eat5 = min((int)five.size(), (w - 3 * eat3) / 5);

    for(int i = 0 ; i < eat5 ; i++) {
        cur += five[pt5++];
        ans = max(ans, cur);
    }
    ...
    while(eat3 > 0) {
        cur -= three[--pt3];
        eat3--;
        int nxteat5 = min((int)five.size(), (w - 3 * eat3) / 5);
        if(nxteat5 > eat5) {
            eat5 = nxteat5;
            cur += five[pt5++];
            ans = max(ans, cur);
